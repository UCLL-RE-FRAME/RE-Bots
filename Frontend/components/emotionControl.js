...
window.customElements.define('emotion-control-ɮ', class extends HTMLElement {
    ...
    connectedCallback() {
        this.socket.addEventListener('open', event => {
            ...
            this.socket.send(JSON.stringify({ "payload": "Hello server, I'll be prodding James today." }));
        });
    ...
    drag(e) {
        ...
        this.dispatchEvent(new CustomEvent('move', {
            ...
            composed: true,
            detail: {
                "source": this.id, "valueX": Math.min(Math.max(0, e.clientX - this.control.left), 300), "valueY": Math.min(Math.max(0, e.clientY - this.control.top), 300)
            }
        }));
        ...
        let currentMatrix = this.selected.transform.baseVal.consolidate().matrix,
